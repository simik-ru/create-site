...
if [ ! -f $SSL_DHPARAMS ];
then
	cp $CURRENT_DIR/dhparams.pem $SSL_DHPARAMS
fi
...
$SED -i "s/@@LOG_PATH@@/\/home\/$USERNAME\/logs/g" $CONFIG
$SED -i "s#@@SOCKET@@#/var/run/"$USERNAME"_fpm.sock#g" $CONFIG

# Copy snippet with SSL params to nginx's snippets directory
SSL_PARAMS=$NGINX_SNIPPETS/ssl-params.conf
if [ ! -f "$SSL_PARAMS" ]
then
	cp $CURRENT_DIR/ssl-params.conf.template $SSL_PARAMS
	$SED -i "s#@@DHPARAMS_PATH@@#$SSL_DHPARAMS#g" $SSL_PARAMS
fi
...
mkdir /home/$HOME_DIR/logs
mkdir /home/$HOME_DIR/tmp
chmod 750 /home/$HOME_DIR -R
chmod 700 /home/$HOME_DIR/tmp
chmod 770 /home/$HOME_DIR/logs
